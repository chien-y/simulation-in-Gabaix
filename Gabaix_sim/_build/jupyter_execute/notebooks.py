...
{cite}'gabaix2011granular' proves that if the variance of firms' sales or sizes is finite, then $\sigma_{GDP}$ decays out as the rate $1/\sqrt{N}.$ Then, the idiosyncratic shocks of firms are averaged out and only have little impact on macroeconomic fluctuation. However, if the sizes of firms follows a power law\footnote{
The variance is infinite if the Pareto exponent is  less than or equal to $2$.}, {cite}`gabaix2011granular` shows that the aggregate volatility decays according to $1/\ln N$ under Zipf's law. Therefore, the GDP fluctuation is substantial even if $N$ is large. The fat tail makes the central limit theorem breaks down and the idiosyncratic shocks remains on aggregate output.
...
When firm's growth rate is $12\%$, the deviation of growth rate $ \sigma_{GDP} = h\sigma $ is around $0.1\%$ for log-normal which is much lower than $0.94\%$ of Pareto when $\zeta=1.059$. Since the empirically measured macroeconomic fluctuation is around $1\%$, the log-normal distribution is not reasonable under this matching method and simulation. 
This also confirms the proposition one of {cite}`gabaix2011granular`, which shows that the GDP volatility decays in the rate of $\frac{1}{N^{1/2}}$ when the firm sizes have finite variance. To conclude, the underlying distribution for firm sizes should be Pareto distribution rather than log-normal distribution in this experiment.  
